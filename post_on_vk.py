...
def get_image_upload_url(vk_page_id):
...
def save_image_on_wall(vk_page_id, uploaded_image):
...
def get_gif_upload_url(vk_page_id):
...
def upload_gif(vk_gif_path, gif_upload_url):
...
def publish_post_in_vk(vk_page_id, media_type=None, saved_file=None, text=''):
